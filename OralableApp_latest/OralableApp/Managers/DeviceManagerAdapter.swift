...
final class DeviceManagerAdapter: ObservableObject, BLEManagerProtocol {
    ...
    private func setupBindings() {
        ...
        // Forward readings to SensorDataProcessor for history storage
        deviceManager.$latestReadings
            .sink { [weak self] readings in
                guard let self = self else { return }
                let sensorReadings = Array(readings.values)
                if !sensorReadings.isEmpty {
                    Task {
                        await self.sensorDataProcessor.processBatch(sensorReadings)
                        await self.sensorDataProcessor.updateLegacySensorData(with: sensorReadings)
                    }
                }
            }
            .store(in: &cancellables)
